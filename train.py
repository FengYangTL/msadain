...
def train(
    ...
    images = build_decoder(output_encoded, weights=None, trainable=True, activation=decoder_activation)

    with open_weights(vgg) as w:
        vgg = build_vgg(images, w, last_layer=encoder_layer, activation=encoder_activation)
...
def build_style_losses(current_layers, target_layers, weight, epsilon=1e-6):
    ...
    for layer in current_layers:
        ...
        mean_loss = tf.reduce_mean(tf.squared_difference(current_mean, target_mean))
        std_loss = tf.reduce_mean(tf.squared_difference(current_std, target_std))
...
if __name__ == '__main__':
    ...
    # training options
    parser.add_argument('--resume',  default=params['resume'],
        type=bool, help='If true, resume training from the last checkpoint')
