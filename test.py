...
def style_transfer(
    ...
    # encoder (vgg19 activation)
    if encoder_activation == 'relu':
        encoder_activation = tf.nn.relu
    elif encoder_activation == 'elu':
        encoder_activation = tf.nn.elu
    else:
        raise ValueError('Unknown activation: ' + encoder_activation)
    ...
    with tf.Session() as sess:
        ...
        for content_path, style_path in product(content_batch, style_batch):
            ...
            if outname_style_first:
                filename = '%s___%s.%s' % (style_name, content_name, save_ext)
            else:
                filename = '%s___%s.%s' % (content_name, style_name, save_ext)
            filename = os.path.join(output_dir, filename)
            save_image(filename, output[0], data_format=data_format)
            print('Output image saved at', filename)


def _build_graph(vgg_weights, decoder_weights, alpha, data_format, encoder_activation, decoder_activation):
    if data_format == 'channels_first':
    ...
    with open_weights(vgg_weights) as w:
        vgg = build_vgg(image, w, data_format=data_format, activation=encoder_activation)
        encoder = vgg['conv4_1']

    if decoder_weights:
        with open_weights(decoder_weights) as w:
            decoder = build_decoder(weighted_target, w, trainable=False,
                data_format=data_format, activation=decoder_activation)
    else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--encoder_activation', default=params['encoder_activation'],
                        help='Activation function in the decoder')
    ...
    parser.add_argument('--outname_style_first', default=params['outname_style_first'], type=bool,
        help="""True if the output filename has style name first""")
